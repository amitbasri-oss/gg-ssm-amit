...
# Let's add the path to the sys.path because otherwise this fails, I need retina folder and then inside of it is training folder
import sys
import os
...
def launch_fire(
    ...
    # LOAD CONFIGS
    if path_to_run != None:
        training_params = load_yaml_config(
            os.path.join(path_to_run, "training_params.yaml")
        )
        dataset_params = load_yaml_config(
            os.path.join(path_to_run, "dataset_params.yaml")
        )
        quant_config = load_yaml_config(os.path.join(path_to_run, "quant_params.yaml"))
        if training_params["arch_name"][:6] == "retina":
            layers_config = load_yaml_config(
                os.path.join(path_to_run, "layer_configs.yaml")
            )
    else:
        ...
        if training_params["arch_name"][:6] == "retina":
            layers_config = get_retina_model_configs(
    ...
    if (quant_params["a_bit"] < 32 or quant_params["w_bit"] < 32) and (
        quant_params["a_bit"] > 1 or quant_params["w_bit"] > 1
    ):
        lsqplusprepareV2(
            model,
            inplace=True,
    ...
    wandb_logger = WandbLogger(
        project=project_name, name=run_name, save_dir=out_dir, mode=wandb_mode
    )
