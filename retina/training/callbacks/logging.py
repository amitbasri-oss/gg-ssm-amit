...
class LoggerOrchestrator:
    ...
    def log_lr(self):
        for i, e in enumerate(self.optimizer.param_groups):
            self.logger.experiment.log(
                {f"optim/lr_{i}": self.optimizer.param_groups[i]["lr"]}
            )

    def log_loss(self, outputs):
        for key in outputs["loss_dict"].keys():
            self.logger.experiment.log(
                {f"{self.dataset_name}/{key}": outputs["loss_dict"][key]}
            )

    def log_snn_scaler_lpf(self):
        self.logger.experiment.log(
            {"model_stats/scale_factor": self.model.scale_factor.item()}
        )
    ...
    def log_snn_statistics(self):
        ...
        for key in layer_stats["parameter"].keys():
            ...
            self.logger.experiment.log(
                {f"{layer_name}/synops_s": layer_value["synops/s"]}
            )
        for key in layer_stats["spiking"].keys():
            ...
            self.logger.experiment.log(
                {f"{layer_name}/firing_rate": layer_value["firing_rate"]}
            )
    ...
    def log_one_visuals(self, data, outputs, epoch, batch):
        ...
        path_to_gif = (
            f"{self.out_dir}/video/{epoch}_{self.dataset_name}_{batch}_{index_vis}.gif"
        )
        ...
        try:
            anim_with_points = plot_animation_points(
                ...
                outputs["memory"]["points"]["target"]
                .reshape(point_shape)[index_vis]
                .detach()
                .cpu(),
                outputs["memory"]["points"]["pred"]
                .reshape(point_shape)[index_vis]
                .detach()
                .cpu(),
            )
...
class LoggingCallback(pl.Callback):
    def __init__(self, logger, model, optimizer, dataset_params, training_params):
        ...
        """
        self.logger = LoggerOrchestrator(
            logger, model, optimizer, dataset_params, training_params
        )
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        self.logger.log_orchestrator(
            ...
            batch=batch_idx,
            outputs=outputs,
            stuff_to_log=["loss", "performance", "stats", "lr"]
            + (["gifs"] if create_gifts else []),
        )
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        self.logger.log_orchestrator(
            ...
            batch=batch_idx,
            outputs=outputs,
            stuff_to_log=["loss", "performance", "iou_metric"]
            + (["gifs"] if create_gifts else []),
        )
