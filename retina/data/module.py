...
class EyeTrackingDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.dataset_name == "DVSGesture":
            transform = tonic.transforms.Compose([tonic.transforms.ToFrame(sensor_size=tonic.datasets.DVSGesture.sensor_size, n_event_bins=self.n_bins), tonic.transforms.NumpyAsType(np.float32)])
